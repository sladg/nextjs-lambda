...
var findObjectInFile = (filePath, regexes) => {
  const content = (0, import_fs.readFileSync)(filePath, "utf-8");
  const data = regexes.reduce((acc, regex) => {
    var _a, _b;
    if (acc) {
      return acc;
    }
    return (_b = (_a = content.match(regex)) == null ? void 0 : _a[0]) != null ? _b : null;
  }, null);
  if (!data) {
...
var executeAsyncCmd = async ({ cmd, path: path3, env }) => {
  ...
  return new Promise((resolve, reject2) => {
    ...
    const sh = (0, import_child_process.exec)(cmd, { env: { ...process.env, ...env } }, (error, stdout) => {
      if (error) {
        reject2(error);
      } else {
        resolve(stdout);
...
var nextServerConfigRegex13_3 = /(?<=nextConfig = )(.*)/;
...
var removeHandler = async ({ appPath, stackName, region, profile }) => {
  ...
  await executeAsyncCmd({
    cmd: `${cdkExecutable2} destroy ${cdkRemoveArgs.join(" ")}`,
    env: variables
  });
};
...
program2.command("pack").description("Package standalone Next12 build into Lambda compatible ZIPs.").option("--config <path>", "YAML config file that can be set as opposed to having to set every flag.").option(
  ...
  if (options.config) {
    ...
    config = js_yaml_default.load((0, import_fs3.readFileSync)(configPath, "utf-8"));
  } else {
    config = options;
  }
...
program2.command("config").description("Create the default config files in the current directory.").action(async () => {
  const configFiles = ["deployConfig.yml", "packConfig.yml"];
  const sourceDirectory = import_path2.default.resolve(__dirname, "./templates");
  configFiles.forEach((configFile) => {
    const source = import_path2.default.resolve(sourceDirectory, `./${configFile}`);
    const dest = import_path2.default.resolve(process.cwd(), `./${configFile}`);
    (0, import_fs3.copyFileSync)(source, dest);
  });
});
